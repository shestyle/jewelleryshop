...
// Kept behavior and accessibility; uses Intl.NumberFormat for INR formatting.
...
function renderProducts(list){
  ...
  list.forEach(p=>{
    ...
    img.src = p.image;
    img.onerror = () => {
      img.src = `https://via.placeholder.com/900x600?text=${encodeURIComponent(p.name)}`;
    };
...
function openModal(p){
  ...
  modalImage.onerror = ()=>modalImage.src = `https://via.placeholder.com/1200x900?text=${encodeURIComponent(p.name)}`;
  document.getElementById('modal-title').textContent = p.name;
  document.getElementById('modal-desc').textContent = p.description;
  document.getElementById('modal-price').textContent = currency.format(p.price);
